        ...
        if (null !== ($type = $this->getCustomType($name))) {
            if (!$this->isMulti($name)) {
                if (!$value instanceof $type) {
                    throw new UnexpectedParamException(sprintf(
                        'Property "%s" has to be %s type, but got %s.',
                        $name,
                        $type,
                        gettype($value)
                    ));
                ...
                foreach ($value as $item) {
                    if (!$item instanceof $type) {
                        throw new UnexpectedParamException(
                            sprintf(
                                'Property "%s" has to be an array of %s type, but one of the elements has %s type.',
                                $name,
                                $type,
                                gettype($item)
                            )
    ...
    private function validateMulti($name, $value)
    {
        if ($this->isMulti($name) && !is_array($value)) {
            throw new UnexpectedPropertyException(sprintf(
                'Property "%s" has to be an array but got %s.',
                $name,
                gettype($value)
            ));
        }
